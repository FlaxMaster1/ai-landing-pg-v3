...
function register({ accessibility = [], cmsNames = [], ...entry }: EntryInput): RegistryEntry {
  return {
...
export const frameworkRegistry: RegistryEntry[] = [
  register({ id: "TOK-color", name: "Color tokens", category: "token", description: "Primitive and semantic color decisions." }),
  register({ id: "TOK-typography", name: "Typography tokens", category: "token", description: "Font-family, weight, and fluid type foundations." }),
  ...
  register({ id: "TOK-breakpoints", name: "Breakpoint tokens", category: "token", description: "Shared content-driven navigation and wide layout transitions." }),
  ...
  register({ id: "CMP-button", name: "Button", category: "component", description: "Action or prominent destination control.", requiredFields: ["content"], variants: ["primary", "secondary", "text"], accessibility: a11yControl, cmsNames: ["Button"] }),
  ...
  register({ id: "CMP-disclosure", name: "Disclosure", category: "component", description: "Native expandable content control.", requiredFields: ["summary", "content"], states: ["closed", "open"], accessibility: a11yControl, cmsNames: ["FAQ Toggle", "Accordion", "Expandable Section"] }),
  register({ id: "CMP-form-control", name: "FormControl", category: "component", description: "Labeled text, email, or textarea control with hint association.", requiredFields: ["id", "label", "type"], variants: ["text", "email", "textarea"], accessibility: a11yControl }),
  ...
  register({ id: "CMP-card", name: "Card", category: "component", description: "Composable media, metadata, title, description, and action card.", requiredFields: ["title"], variants: ["editorial", "promotional", "navigation", "feature", "compact", "media-led", "text-led"], cmsNames: ["Classic Tile", "Rowhouse Tile", "Page Tile", "Info Box"] }),
  register({ id: "CMP-story-card", name: "StoryCard", category: "component", description: "Semantic Story entity presentation.", requiredFields: ["story"], variants: ["editorial", "compact", "feature"], cmsNames: ["Story Tile"] }),
  ...
  register({ id: "PAT-hero", name: "Hero", category: "pattern", description: "Page-opening composition that owns the page H1.", requiredFields: ["id", "heading"], variants: ["short", "tall", "media"], accessibility: ["Must be the only H1 owner when titleMode is hero."], cmsNames: ["Hero Header"] }),
  register({ id: "PAT-page-intro", name: "PageIntro", category: "pattern", description: "Introductory supporting content beneath a template-owned title.", requiredFields: ["id", "text"], accessibility: a11yText }),
  register({ id: "PAT-card-grid", name: "CardGrid", category: "pattern", description: "Responsive collection of Card components.", requiredFields: ["id", "items"], variants: ["editorial", "promotional", "navigation"], cmsNames: ["Classic Tiles", "Rowhouse Tiles"] }),
  register({ id: "PAT-feature-row", name: "FeatureRow", category: "pattern", description: "Promotional media and content composition.", requiredFields: ["id", "heading", "text"], variants: ["media-start", "media-end"] }),
  ...
  register({ id: "PAT-faq", name: "FAQ", category: "pattern", description: "Question-and-answer group composed from native Disclosure components.", requiredFields: ["id", "items"], accessibility: ["Each question is a keyboard-operable native summary."], cmsNames: ["FAQ Toggle"] }),
  register({ id: "PAT-tabs", name: "Tabs", category: "pattern", description: "Locally stateful tablist and panel composition.", requiredFields: ["id", "items"], variants: ["horizontal", "vertical"], states: ["selected", "unselected"], accessibility: ["Maintain tab, tablist, and tabpanel relationships plus arrow/Home/End keyboard behavior."], cmsNames: ["Horizontal Tabs", "Vertical Tabs", "New Tabs"] }),
  ...
  register({ id: "PAT-form", name: "Form", category: "pattern", description: "Validated form presentation configured for a provider boundary.", requiredFields: ["id", "heading", "fields"], states: ["idle", "invalid", "success"], accessibility: ["Associate labels and hints, use native validation, and announce result state."], cmsNames: ["Gravity Form", "Campaign Monitor Form", "Pardot Form"] }),
  ...
  register({ id: "GBL-global-header", name: "GlobalHeader", category: "global", description: "Shared program, identity, primary, mobile, and search shell.", requiredFields: ["site"] }),
  register({ id: "GBL-global-footer", name: "GlobalFooter", category: "global", description: "Configured additional, institutional, and legal link groups.", requiredFields: ["site"], accessibility: ["Use a named footer landmark and semantic grouped headings."] }),
