...
export const frameworkRegistry: RegistryEntry[] = [
  ...
  register({ id: "CMP-quote-card", name: "QuoteCard", category: "component", description: "Named quote presentation with an optional photo, name, and title.", requiredFields: ["name", "quote"], accessibility: a11yMedia, cmsNames: ["Quote Block"] }),
  ...
  register({ id: "PAT-quote-grid", name: "QuoteGrid", category: "pattern", description: "Fixed two-column collection of QuoteCard components that does not collapse to a single column on narrow viewports.", requiredFields: ["id", "items"], relationships: { contains: ["CMP-quote-card"], mayContain: [], dependsOn: [], usedBy: [], relatedTo: [], replaces: [] } }),
